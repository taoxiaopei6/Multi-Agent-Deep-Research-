"""Configuration loader: builds AppConfig from .env and config.json."""
...
class AppConfig:
    ...
    def from_file(path: str | Path | None = None) -> "AppConfig":
        ...
        if not api_key:
            raise ValueError(
                f"DASHSCOPE_API_KEY not configured，请在 {config_path} 中填写 api_key，或设置环境变量 DASHSCOPE_API_KEY"
            )
        model = AppConfig._resolve_str(data, "model", "MODEL", "qwen-plus")
        base_url = AppConfig._resolve_str(data, "base_url", "BASE_URL", "")
        embedding_model_path = AppConfig._resolve_str(data, "embedding_model_path", "EMBEDDING_MODEL_PATH", "")
